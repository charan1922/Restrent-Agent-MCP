  ...
  inputSchema: {
    ...
    properties: {
      category: {
        type: "string",
        description:
          "Optional category filter (appetizers, mains, desserts, drinks)",
      },
    },
    required: [],
  },
...
class RestaurantMCPServer {
  ...
  private setupToolHandlers(): void {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        switch (name) {
          case "get_menu":
            return await this.handleGetMenu(args as { category?: string });
          ...
          case "check_table_status":
            return await this.handleCheckTableStatus(
              args as { tableNumber?: number }
            );

          case "make_reservation":
            return await this.handleMakeReservation(
              args as {
                customerName: string;
                partySize: number;
                date: string;
  ...
  private async handleGetMenu(args: { category?: string }) {
    const menu = {
      appetizers: [
        {
          name: "Bruschetta",
    ...
    if (category && menu[category as keyof typeof menu]) {
      result = menu[category as keyof typeof menu];
      return {
        content: [
          {
            type: "text",
  ...
  private async handlePlaceOrder(args: {
  ...
  private async handleCheckTableStatus(args: { tableNumber?: number }) {
    ...
    if (tableNumber && mockTables[tableNumber as keyof typeof mockTables]) {
      const table = mockTables[tableNumber as keyof typeof mockTables];
      return {
        content: [
          {
            type: "text",
      ...
      const allTables = Object.entries(mockTables)
        .map(
          ([num, table]) =>
            `Table ${num}: ${table.status} (${table.guests} guests)${
  ...
  private async handleMakeReservation(args: {
    customerName: string;
    partySize: number;
    date: string;
    time: string;
